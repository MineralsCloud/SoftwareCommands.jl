...
[`SoftwareCommands`](https://github.com/MineralsCloud/SoftwareCommands.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("SoftwareCommands")
   ```

3. Run

   ```julia-repl
   julia> using SoftwareCommands
   ```
...
version of `SoftwareCommands`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/SoftwareCommands.jl"
```

in the second step above.

## Update `SoftwareCommands`
...
our [GitHub repository](https://github.com/MineralsCloud/SoftwareCommands.jl)
for new releases.
Once we release a new version, you can update `SoftwareCommands` by typing

```@repl
using Pkg
Pkg.update("SoftwareCommands")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `SoftwareCommands`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("SoftwareCommands")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `SoftwareCommands`.
