...
!!! info
    The style guide is always a work in progress, and not all SoftwareCommands code
    follows the rules. When modifying SoftwareCommands, please fix the style violations
...
`SoftwareCommands` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/SoftwareCommands.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `SoftwareCommands` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `SoftwareCommands` have
